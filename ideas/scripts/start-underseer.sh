#!/bin/bash
# Start the underseer daemon and create the daemon-enabled flag.
...
UNDERSEER_PY="/var/www/hdp/agents/overseer/underseer.py"
LOG="/var/www/hdp/agents/overseer/underseer.log"

# Check if daemon is already running
if pgrep -f "python3.*underseer.py" > /dev/null; then
    echo "Underseer daemon is already running (PID: $(cat /var/www/hdp/agents/overseer/underseer.pid 2>/dev/null))"
    exit 0
fi
...
sleep 1
DAEMON_PID=$(pgrep -f "python3.*underseer.py" | head -1)
echo "Underseer daemon started (PID: ${DAEMON_PID})."
echo "Log: tail -f $LOG"
